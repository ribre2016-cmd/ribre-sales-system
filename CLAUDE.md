...
1. **Vercel Hobbyは Serverless Functions 12個まで**。現在ちょうど12個。**新しいAPIを増やすときは既存への統合が必須**（evidence-action.js が統合例）
...
- 毎日 UTC22時（JST朝7時）: `/api/mf/auto-match` — 自動マッチング＋結果があればSlack通知
