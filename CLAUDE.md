...
22. **仕訳を削除しても、元の明細は未仕訳に戻らない。「対象外」になる**（P7-Bとして本番で実測・2026-08-05）。
    ...
    **対象外になった明細があると「完了」に見えてしまう**。→ **2026-08-05 対応済**: ⑨に「対象外にされた明細」を出す（`fetchTransactionsByStatus`・
    `txwRenderMonthlyExcluded`・`tools/test-excluded.js`）。対象外があるときは
...
26. **`recordAction` は必ず `res.ok` を見ること。列名は `actor_email`**（2026-08-06のレビューで発覚）。
    `handleMonthlyCheckConfirm` だけが `advisor_email` と書いており、存在しない列＋NOT NULL違反で
    INSERTが必ず拒否されていた。ところが `recordAction` は fetch の例外しか拾っておらず、
    **拒否されても成功扱い**。画面は「操作履歴に記録しました」と出し続け、
    ⑤月次チェックの「確認しました」は**一度も記録されていなかった**。
    → `recordAction` は true/false を返す。呼び出し側は失敗を画面に伝えること。
    あわせて、招待の発行・取り消し・税理士の有効無効にも記録が無かったので追加した
    （`tax_advisors` に履歴列が無いため、記録が無いと誰がいつやったか永久に分からない）。

27. **⑤月次チェックの集計に、提案用に取った仕訳を使い回さないこと**（2026-08-06）。
    事業区分別の課税売上高が、
    (1) 未仕訳0件の月では**表ごと消える**（提案用の取得が「未仕訳があるとき」しか走らないため）、
    (2) 未仕訳が残る月は**最後の未仕訳の日付で仕訳が切られ**、それ以降の売上が抜ける、
    という2つの穴があった。集計は対象月の**末日**まで取り直す。

28. **比較対象が足りない月は「判定していない」と明示すること**（2026-08-06）。
    事業年度が3月始まりで推移表はその期の月しか返らないため、**3〜6月は過去4か月が揃わず**
    ①②とBSの凍結・急変の判定が実行されない。画面は「該当する科目はありません」と出しており、
    「判定した結果0件」と区別できなかった（制約20と同じ話）。
    → 応答に `lookback_available` / `lookback_enough` を載せ、足りないときは画面で言い切る。

29. **選択候補と一致しない入力は、必ずサーバー側でも弾くこと**（2026-08-06）。
    税区分は画面のガードが無く、「課税仕入10」のように途中まで打った文字が残っていても
    登録でき、**税区分だけ黙って未指定で送られていた**（勘定科目とインボイス区分は弾いていた）。
    消費税に直結し、登録した仕訳はこの画面から取り消せない（制約22）。
    → 画面は `tax_text`（打った文字）を必ず添えて送り、サーバーは
    「打ったのに `tax_id` が無い」なら `tax_not_resolved` で断る。

30. **「今日」は必ず `todayJst()` で数えること。UTCの生日付を使わない**（2026-08-06）。
    `isMonthInProgress` / `isInProgressingTerm` がUTCで判定しており、
    **月初の午前9時までは前月がまだ「途中」扱い**になっていた。その間、計上漏れ・
    少ない側の異常値・BSの警告が**まるごと隠れる**。同じファイルのログイン通知はJSTで
    数えており、1ファイル内で基準が食い違っていた。

31. **一覧を読み直す関数には世代番号を持たせること**（2026-08-06）。
    `txwLoad()` に非同期ガードが無く、対象月を素早く切り替えると
    **古い月の応答が新しい月の画面を上書き**していた（月セレクタは7月なのに中身は6月）。
    MFのAPIはレート制限と再試行があるため、先に投げた方が先に返るとは限らない。
    提案の描画(`txwRenderGen`)は同じ守り方をしていたのに、主系統だけ素通しだった。

32. **閲覧範囲の基準は、画面とCSVで必ずそろえること**（2026-08-06）。
    ④操作履歴は画面が `isMember`、CSVが `isAdmin` で判定しており、
    税理士側の管理者が**画面で見えないものをCSVでは全件取得**できた。
    `tax_advisors` に事務所を区別する列が無いため、事務所を増やすと他事務所の分まで見える。
...
3. ~~運賃明細の伝票番号10桁73件~~ → **2026-08-02 調査完了・対処不要**。
   正体は伝票番号ではなく `2026/03/25` 等の**日付文字列**（10文字）。生データの先頭が
   `o11962471` 等のヤフオク商品IDだったことから、**3〜4月頃に売上CSVを配送CSVとして
   誤取込した残骸**と判明（当時は種類の自動判定が無かった）。運賃0・商品IDなし・
   突合0件で、伝票欄が日付形式のため将来も一致しない。画面の表の不一致件数にも
   含まれない（表は売上行から数えるため）。影響は再照合後のメッセージの数字のみ。
   クラウド同期テーブルには安全上わざとdeleteポリシーを付けていないため、
   ローカルだけ消しても次のpullで戻る。**無害なので触らない判断**（利用者了承済み）
4. **常駐版**: ✅ 2026-08-02 構築済み（`tools/evidence-watcher.ps1` +
   `evidence-watcher-install.ps1` + `docs/EVIDENCE_WATCHER_SETUP.md`）。
   **利用者のPCでのインストール作業が未実施**（MAIL_INGEST_SECRETの設定＋
   タスクスケジューラ登録）。証憑のみ対象・承認制は維持・MFへの自動送信はしない
5. **MF仕訳の自動作成**: 設計書 `docs/MF_JOURNAL_PLAN.md` に加え、2026-08-02に
   モック画面 `mockups/mf-journal-mock.html` とUI仕様書 `docs/MF_JOURNAL_UI_SPEC.md`
   を作成済み（実装してはいけないことリスト21項目つき）。
   **実装着手の前提は変わらず**: 税理士への確認14項目の回答＋消費税の丸め実測(A1〜A3)＋
   journal.write再認可。前提が揃うまで実装しない
