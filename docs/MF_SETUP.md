...
5. テスト画像を貼り付け、送信ボタンで `/api/mf/vouchers` にPOSTし、`{ok:true}` が返ることと
   `mf_evidence` テーブルに行が追加されることを確認する。送信時点で一致するMF仕訳が
   見つかった場合のみ `status='attached'`（1回のPOSTで添付済み）。通常のテストでは
   一致する仕訳が無いはずなので `status='awaiting_match'`（マッチ待ち）になる。
   このままMFへは送信されず、日次の自動マッチング（`/api/mf/auto-match`。手動の
   「マッチング実行」でも同様）が見つかるまで再チェックし続ける（`api/mf/_lib/mf-match-core.js`
   の `processAwaitingMatch`）。自動フォールバックは無いため、長期間見つからない場合は
   台帳から手動で対応する（削除して再登録、またはMF画面から直接対応）。
...
併せて awaiting_match（マッチ待ち）状態の証憑もリトライする（`processAwaitingMatch`）。一致すれば
1回のPOSTで添付済み送信（二重アップロードなし）。一致しない間はMFへは送信せず待ち続ける
（自動フォールバックは無し。ユーザー判断で長期未マッチのものへ手動対応する運用）。
