...
function buildOcrPrompt() {
  ...
  return (
    ...
    '出力は必ずJSONオブジェクト1つのみ。配列(例: [{...},{...}])は絶対に返さない。複数の値を候補として並べることも禁止。' +
    '精算書・請求書・納品書明細のように商品明細や中間の小計（個々の商品ごとの金額、買主小計、消費税、手数料の内訳等）が' +
    '複数ページ・複数箇所に渡って書かれていても、それらを合算したり個別に返したりしない。' +
    '書類の中で最終的に支払う/受け取る1つの金額（通常は書類の一番下や合計欄にあり、' +
    '「合計」「合計金額」「精算合計金額」「お支払い金額」「ご請求金額」等とラベルが付いている、太字や大きい文字のことが多い）' +
    'だけをamountに入れる。それ以外の小計・個々の商品明細の金額は無視する。迷ったら書類の中で最も大きい確定額ではなく、' +
    '「合計」の文字が付いた最終値を優先する。' +
    ...
    'amountは証憑に印字された数値をそのまま返す（円換算・為替換算は絶対にしない）。マイナス表記の場合は符号を外した絶対値で返す。' +
...
function extractOcrJson(text) {
  ...
  // ことで発生自体を減らす方針。buildOcrPromptを参照）。
