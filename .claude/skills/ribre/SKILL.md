...
| 粗利の明細(売上/仕入) | `ribre_smp_profit_meisai_v1` | app_settings `skey='profit_meisai'` | **行単位マージ**(2026-07-02〜。各行`up`＋削除は`tomb`墓標180日。push=GET→merge→POST、pull=merge＋欠け行を押し戻し。`smpMeiMerge`) |
| 仮入力(当月チャネル/送料) | `ribre_smp_profit_prov_v1`(+`_ts`) | app_settings `skey='profit_prov'` | **マス単位マージ**(月×チャネルごと`_m`更新時刻で新しい方。`smpProvMerge`) |
| 登録先プルダウン | `ribre_smp_partners_v1` | (なし。候補は同期済み明細名から生成) | 明細経由で共有 |
- 明細/仮入力の手動同期=粗利タブ「🔄 他の端末と同期」(`smpProfitSyncNow`)。マージ方式なので**順序不問・入力は消えない**。push成否を表示(404=app_settings未作成/401=再ログイン/403=RLS)。visibilitychangeで画面復帰時に自動pull(30秒に1回)。旧LWW時代に削除した明細は古い端末から一度復活しうる→再削除すれば墓標で全端末維持。
- ホーム=全体ダッシュボード(全合算)、**集計タブも全体**(明細・仮入力含む。simpleRenderSummaryがsmpProfitMonthTotalsベース／内訳表はEC分)、売上一覧ページ=EC集計ダッシュボード+EC一覧、粗利タブ=年間表+明細入力。月ロック(`ribre_smp_locked_months`／app_settings `locked_months`)はCSV取込からロック月を保護。全データバックアップ=`smpFullBackup`/`smpFullRestore`(明細・仮入力・登録先・ロックも含む)。**目標機能**=ホームの🎯カード(年度/月ごとボタン切替・全体対象・売上/粗利目標・「あと何個」=残÷単価。先月まで=件数から平均、当月=手入力。`ribre_smp_goals_v1`／app_settings `goals`でts同期。smpRenderGoals)。詳細は別メモ参照。
