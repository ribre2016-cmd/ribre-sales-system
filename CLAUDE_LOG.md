...
## 2026-08-04 Phase 7 実装（⑨月次チェック・①一覧表示・承認ワークフロー）

設計だけだったものを本番へ実装した。**モックだけをレビューさせていたせいで
見つけられなかった不具合**が、実データで動かして次々に出た。以下は実装の記録と、
その過程で分かったこと。

### 実装したもの
- **⑨月次チェック**（読み取り専用）: 推移表から計上漏れ・金額異常・符号異常。
  未仕訳の残件数を先頭に出し、登録が途中の月は参考表示に留める。
  「このチェックで見つけられないもの」を併記。確認の記録は画面とサーバーの両方でチェック必須
- **①一覧表示**: 口座・カードごとにまとめて表示（既定は閉じる）。行内編集・20行上限・
  一括確認（実行は1件ずつ・サーバー無変更）・Shift範囲選択・全部開く/閉じる
- **承認ワークフロー**: `approval_policy='required'` で担当者の登録は承認待ちへ。
  承認時に依頼時点の明細と突き合わせ、変わっていたら実行しない
- **admin/staffの役割**、**操作履歴のCSVダウンロード**
- 配色をRIBRE共通トークン(hallmark.css)へ統一

### 実データで初めて分かった不具合（重い順）
1. **勘定科目の提案が本番で常に0件だった**。原因は3つ重なっていた:
   - MFの `GET /journals` は**会計年度をまたぐ期間で400**（CLAUDE.md 制約18に記載）
   - 会計期間対応の修正で `startDate` の宣言を消したのに応答で参照し **ReferenceErrorで500**
   - 画面が「提案0件」と「取得失敗」を**同じ文言**で出していたため、
     不具合が『該当する提案はありません』に化けて**原因を3回取り違えた**
   → 失敗は必ず理由を残す。取得失敗は赤字で区別する
2. **要確認の黄色が偶数行で消えていた**（新人レビューが実機で発見）。
   縞模様 `.list-table tr:nth-child(even) td`（詳細度0,2,2）が
   `tr.txw-lt-lowmatch td`（0,1,2）に勝っていた。登録済みの緑も同じ
3. **証憑・共有ファイルも取得失敗を空配列で返していた**（提案とまったく同じ形）
4. **CSVの「全期間」が limit=5000 で黙って打ち切り**、しかも件数を出していなかった。
   所長レビューの言葉「全部揃っていると思って提出したら古い分が抜けていた」
5. **「過去1件」だけの提案が44件一致と同じ見た目**だった。割合は1/1でも100%になるため
   6〜8割の要確認条件に引っかからない。『税金→預り金』のように摘要が同じでも
   中身が毎回変わる科目が危ない（新人・所長が独立に指摘）

### SQLの誤り
`tax_workspace_settings` は既に「id=1の1行だけを持つ設定テーブル」として存在していたのに、
確認せず key/value 型の別テーブルとして書き、利用者の実行が失敗した。
**新しいテーブル・列を書く前に、必ず既存のスキーマを確認すること。**

### 実測（アプリのトークン・読み取りのみ）
- P7-D `report.read` ○ / offices.read ○
- P7-A `voucher_file_ids` **×**（明細側は構造的に常に空。証憑は仕訳にしか付かない）
- P7-C **手動管理の口座が無い**（⑦を作るならMFの画面で作る必要）
- P7-B **未実測**（本番の帳簿に書くため許可待ち）

### レビュー（本番の実装に対して）
新人 B→**A** / 中堅 **A**（削減時間を月15分→8〜10分に自ら下方修正）/ 所長 B→B（残り1点＝CSVの件数表示。修正済み）

### 追加した自動テスト
`tools/test-suggest.js`（提案）/ `tools/test-monthly-check.js`（月次チェック）/
`tools/test-approval.js`（役割・承認・CSV）

### 利用者の作業
- `supabase_tax_approval.sql` 実行 → **完了（2026-08-04）**
- 残り: P7-Bの許可、⑦現金払いの方針（現金の記帳方法の確認）、
  admin にする人の指定、承認を必須にするかの判断

## 2026-08-04 Phase 7 設計（MFの補助への再定義・3視点レビュー3巡で全員A）

実装はしていない。設計書 `docs/TAX_WORKSPACE_PHASE7_PLAN.md`、
実測手順書 `docs/TAX_WORKSPACE_PHASE7_MEASURE.md`、
モック `mockups/tax-workspace-phase7-mock.html` のみ。

### きっかけ（利用者の指摘）
「MFを作りたいんじゃなくてMFの補助。税理士は顧客から資料をもらって登録する
のだから、その最大限の補助を」

Phase 1〜6 は「MFの未仕訳明細をこの画面から登録する」＝MFの中核をなぞる方向で、
MFより速くなることが構造的に難しかった。中堅レビューが毎回「MFで直接やったほうが
速い」と言い続けていたのはそのため。

### MF API の棚卸し（実測・読み取りのみ）
openapi.yaml 全文(213,481文字)を取得し **全23エンドポイント**を数えた。**使用10・未使用13**。
MF公式MCP経由で実データを照会して検証:

- **`reports/transition_pl`**（未使用・要 `report.read`）で
  **「いつもあるのに今月まだ無い科目」を機械的に検知できる**。
  実測: 地代家賃198,000×4ヶ月→7月0、減価償却費369,759×4→0 など**7科目**。
  金額異常(支払手数料5月221,007＝中央値の約7倍)・符号異常(諸会費3月−16,760)も検出
- **`term_settings`**（要 `offices.read`）で会計年度3/1〜2/28・税込経理・簡易課税・
  切り上げ・卸売＋小売が取れる。ハードコードしていた前提が全部裏取りできた。
  **ただし「その期が締まっているか」のフラグは無い**→決算済み期の警告は手動設定のまま
- **`DELETE /journals/{id}` が存在する** →「登録したら取り消せません」は誤りだった。
  ただし証憑添付済みは対象外(MFの証憑は取り消せずBoxに孤児ファイルが残るため)
- **`POST /transactions`** で現金払いを手動口座の明細として作れる
  → Phase 5 の仕訳直接作成をやめ既存フローに乗せる。**P5-A/B/C(消費税の丸め)が不要に**
- **`transactions.voucher_file_ids`** で資料が来ていない明細を出せる(実測は未)

### 設計の柱
MFが担うのは「③仕訳を登録する」だけ。**①資料を集める・④顧客に質問する・
⑤月次で確認する が丸ごと空いている**。しかもこのシステムは**すでに顧客側の画面**で、
税理士向けSaaSが構造的に持てない資産がある。

新設: ⑨月次チェック(読み取りのみ) / ⑩資料の受け取り / **質問と回答**(中核) /
①登録の取り消し。「MFでやったほうが速いことはMFへのリンクを出して終わり」を明文化。

### 3巡のレビューで見つかったもの
- **設計の穴(新人)**: 「7月0円は月の途中か計上漏れか区別がつかない」
  → 月次チェックの先頭に**未仕訳明細の残件数**を出しデータで区別。進行中の月は判定しない
- **見落とし(中堅)**: CLAUDE.md に既にある「現金レシートはMFクラウド経費アプリ
  (守備範囲外)」との重複 → ⑦を「作るか決める前に確認すること」へ格下げ
- **実装の不備(新人・所長が独立に発見)**: 「全科目を確認しました」の記録ボタンが
  **チェック未ONでも押せた**。所長が最も懸念した「思考停止」が記録つきで再現していた
  → disabled＋関数側の二重ガード。**サーバー側検証も実装要件に**
- **開示の不正確(所長)**: 「止まったとき失われるもの」に**④操作履歴**が抜けていた。
  誰が承認して登録したかという事務所の説明責任の記録も同じDBにしかない
- **設計原則に格上げ**: 「この判定で見つけられないもの」を判定条件と同じ大きさで併記
  （機械が何かを出す画面は「出なかったこと」を安全の証明だと誤解させる）

### 結果
新人 B→B→A / 中堅 B→**A** / 所長 B→B→**A**。**設計とモックは全員合格**。
中堅の見積もり: 1社あたり**月15分**(担当20社で月5時間相当)。

### 残っているもの（設計では埋められない）
実測 P7-A〜D。**利用者にお願いするのは3つ、うち必須は「MF連携のやり直し1回」だけ**。
これだけで⑨⑩質問と回答（工程①④⑤）が進められる。

## 2026-08-04 Phase 6 設計（3視点レビュー→全面反映→再レビューで解消確認）

実装はしていない。設計書（`docs/TAX_WORKSPACE_PHASE6_PLAN.md`）とモック
（`mockups/tax-workspace-phase6-mock.html`）のみ。

### 経緯
1. 新人・中堅・所長の3視点でレビュー（それぞれ実際に画面を操作させた）
2. 指摘を**すべて採用する**利用者判断 → Phase 6 設計書とモックを作成
3. **同じ3人に再レビュー** → 前回の重大指摘はすべて解消判定
   - 新人「11項目中8解消」／中堅「100件の月でも使える見込みが立った」
   - 所長「承認制ONなら限定した職員への展開を検討してよい段階」

### 設計の柱
- **一括確認リスト**: §9-2「一括登録禁止」を利用者判断で条件付き改訂。
  確認は一覧・**実行はサーバー無変更で1件ずつ**・全選択なし・20行上限
- **事務所の管理**: admin/staff役割・事務所側での招待/無効化・
  承認ワークフロー（設定制・既定OFF）・履歴の全件閲覧とCSVエクスポート
- **文言と用語**: 用語表・貸方の使い分け・証憑不要の判断例・
  「不安なら聞く」導線・提案の根拠説明強化
- タブ2グループ化・できないことパネルは畳める

### 見つかった設計上の誤り（モック段階で発見できたもの）
- 貸方の例に「事業主借」→ **個人事業主の科目で法人には不適切**。役員借入金等に修正
- 一致率40%の行に提案が入ったサンプル → 6割閾値と矛盾。「提案なし」の実例に修正

### 再レビューで追加された本実装時の必須要件（設計書§10）
失敗カウントはエラーのみ／20行上限はチェック時点で／行ごとのクリア／
低一致率の視覚強調／Shift+矢印の範囲選択／承認実行時の内容突き合わせ(P6-B解決)／
承認・差し戻しの履歴とCSV記録／依頼者への結果通知／履歴に取引金額列／
用語説明を全タブ共通リンクに

### あわせて本番へ反映した改善（レビュー1巡目から）
- タブをstickyで固定・登録後に次のカードへフォーカス移動・「未仕訳◯件」の即時更新
- 操作履歴に勘定科目・税区分・証憑の列を追加
- 文字サイズの底上げ（本文15px/1.7。モック・本番両方）

## 2026-08-03 税理士ワークスペース Phase 4（過去の仕訳から初期値を提案）を実装

`action:'suggest'` を追加。過去365日の仕訳を読み、明細の取引内容と語が共通する仕訳から
「借方の勘定科目・税区分・補助科目・インボイス区分」の組み合わせを頻度で集計して返す。

**なぜ必要か**: MFの「自動で仕訳」画面には元々この機能がある（Phase 2の実測で、
MFが自動提案した値とこちらが過去の仕訳から選んだ値が完全に一致した）。
これが無いとこの画面はMFより入力が面倒になるため、**追いつくための機能**。

### 「前回と同じ」は「正しい」ではないので、迷ったら提案しない
- 同じ取引先で組み合わせが割れている（最多が6割未満）→ 提案しない
- 借方が複数ある複合仕訳 → 参考にしない
- 過去に一度も出ていない取引先 → 提案しない
- 法人格・汎用語だけの一致（`vendorTokens` のstopword）→ 提案しない
- IDがマスタに無いときは画面の入力欄を**空のまま**にする（それらしい文字列を入れない）

### 画面
- 提案が入ったカードには必ず根拠つきの注意書きを出す
  「前回の仕訳から入れています。確認してください。（過去5件中3件・最終 2026/7/15）」
- 「クリア」で4項目を空に戻せる（一から選び直せる）
- **提案が入っていても自動では登録しない**。必ず人が押す
- `list` の描画をブロックしない（明細を先に出し、提案は後から入る）

### 壊れにくさ
仕訳の取得に失敗・権限不足・応答が壊れている・通信エラーのいずれでも、
明細は表示され入力欄は空のまま、JSエラー0件であることを実機で確認。
提案は「あれば便利」なだけなので、取れないことを理由に登録作業を止めない。

テスト17項目（サーバー）＋実機確認（画面）。
...
### 未確認事項Eを「設定」にして解決（同日追記）
当初は「決算済みの期にMFが登録を拒否するか」を税理士へ確認する予定だったが、
**税理士自身に画面で選んでもらう設定**にすることで確認待ちを無くした。

`tax_workspace_settings.closed_term_policy`（id=1固定・RLSポリシー無し＝service roleのみ）
- `warn`（既定）… 警告を出すだけで登録はできる
- `block` … 進行中の期以外への登録を拒否する

⚠ **判定はサーバー(`handleJournalize`)で必ず行う。** 画面にも同じ判定を入れて
事前に伝えるが、画面のガードは迂回できるため、そちらだけで守っているつもりにならないこと。
拒否したときは対象の期（年度・期間）を返し、画面で具体的に案内する。拒否も操作履歴に残す。
...
実際の明細1件（PayPay銀行8828331 `2026-07-22 振込手数料 145円`）を
MFの画面で仕訳登録し、その仕訳をAPIで読んで確認（仕訳No.751）。

### 最大の成果: 消費税の計算はMFがやる（未確認事項A）
145円 → 借方 支払手数料 **本体131 + 消費税14** / 貸方 普通預金 145。
145 ÷ 1.1 = 131.818… に対し本体を切り捨てて131、消費税は差額の14。

**これで `docs/MF_JOURNAL_PLAN.md` の最重要課題（消費税の丸めをアプリ側で
再現できるか＝A1〜A3の実測待ち）を回避できることが実証された。**
`POST /transactions/journalize` の経路では**アプリは消費税を一切計算しない**。

### そのほかの結果
- **B**: MFの仕訳帳の「⋮」に「編集」「削除」の両方がある。間違えても税理士が画面で直せる。
  Phase 1〜3で取り消し機能を作らない判断は妥当と確認。
  ⚠ `DELETE /journals/{id}` API自体は未検証（実際の仕訳を消す必要が無かったため）
- **C**: 画面では `適格` が自動で入る。ただし**APIで省略したときの既定値は未確認**。
  → Phase 3では `invoice_kind` を**必ず明示送信する**設計にして依存しない
- **D**: `trade_partner_code` は `GET /trade_partners` の `code`。実際は取引先0件で、
  摘要に取引先名を入れる運用のため**取引先の選択欄は作らなくてよい**
- **F**（xlsxを証憑にできるか）: 利用者判断でExcelは証憑にしないため**検証不要**。
  MFへ証憑を送る操作は取り消し不能なので、やらずに済んだのは安全面でも良い
- **H**（ファイルサイズ上限）: 保留。証憑添付は既に本番稼働中で形式もPDF・画像に確定
- **G**（収入側）: 仕組みは支出側と同じ（`side`はリクエストに含まれずMFが明細から判断）
  のため事前検証は不要とし、Phase 3の初回で確認する
- **E**（決算締め後の期の扱い）: **税理士への確認が残る**

### 観測できたこと
- 摘要には明細の `content` がそのまま入る（Phase 3の照合に使える）
- 相手勘定と補助科目はMFが連携口座から自動で決める。アプリは指定しない
- MFの画面が自動提案した科目・税区分・インボイス区分と、こちらが過去の同種仕訳から
  選んだ値が**完全に一致**した。§3.3「前回と同じ科目を初期値にする」方針の裏付け

### 手順の記録
- 環境の安全機構により、AIからのMF書き込み（`postTransactionJournalize`）はブロックされた。
  自分に権限を与える設定ファイルの作成も同様にブロックされた（正しい動作）。
  そのため**利用者がMFの画面で登録し、結果をAPIで読み取る**方法に切り替えた
- MFのMCPには**仕訳を削除するツールが無い**（作成・更新のみ）。
  仮に書き込みを許可しても取り消せないため、この進め方が結果的に安全だった

**Phase 3の着手条件は「税理士の了解」だけになった。**
...
### 招待リンクと社内メンバーのアクセス（同日追記）
- 税理士のメールを事前に聞かずに済むよう **招待リンク方式** を追加（`tax_advisor_invites`）。
  **誰でも自己登録できる形にはしない**（Phase 3で帳簿への書き込み権限を持つ画面のため）。
  有効期限7日・1回だけ使える・発行者がいつでも取り消せる。
  同時に2人が同じリンクを開いても、条件付きUPDATEでclaimするので片方しか成功しない
- **登録されるメールは必ずログイン中のアカウントのもの**を使う。
  リクエストの中身にemailを入れても無視するので、他人を勝手に登録できない
- 社内メンバー（`MEMBER_EMAILS`）は `tax_advisors` に無くてもこの画面を使え、
  招待の発行・取り消しと税理士の有効/無効の切り替えができる（`is_member: true`）
- 認可のテスト24項目を作成（部外者は403でデータを一切返さない／招待は2回使えない／
  期限切れ・取り消し済みは使えない／他人のメールを指定しても無視される 等）

### 利用者側の作業（未実施）
1. `supabase_tax_advisor.sql` を実行（招待テーブルを含む）
2. MFを再連携（スコープ追加のため）
3. 税理士ワークスペースを開いて招待リンクを発行し、税理士へ渡す
4. 税理士へ「この画面で仕訳登録まで行ってよいか」を確認（設計書§7）

## 2026-08-02 Excel年間集計シート＋証憑の重複対策

### Excel出力に「年間集計」シート（1番目）とグラフを追加
- `appvBuildYearSummarySheet`: 年度（3月〜翌2月）の月ごと横並び表。
  売上/税抜売上/消費税/仕入/手数料/送料/利益/商品数/平均単価 × 各月 ＋ 合計列。
  各月の数値は月別シートと同じ `appvMonthReportStats()` を通すので両者がずれない。
  合計の平均単価だけは月平均の合計ではなく `売上合計 ÷ 商品数合計`。
- グラフ2点（折れ線「売上と利益の推移」・棒「費用の内訳」）を追加。
  `appvBuildXlsx` はもともと数値と文字列だけの最小OOXMLライターだったため、
  drawing/chart パートの生成を追加した（`appvXlsxBuildChartXml` ほか）。
  **`<drawing>` は `</sheetData>` の後**でないとExcelが修復ダイアログを出す。
- 検証: Excel COM で実際に開き、修復ダイアログなし・シート順（年間集計が先頭）・
  `ChartObjects().Count = 2`・合計値が月別シートと一致することを確認済み。
- 「全期間」出力は年度に紐づかないため従来どおり単一シート（変更なし）。

### 証憑の二重登録を防ぐ判定期間を24時間→400日に拡大
- 症状: 同じVercelの請求書（2026-07-13 / 20USD）が7/14と7/19の2件登録された。
- 原因: `findRecentSemanticDup` の照合範囲が `created_at >= 24時間前` に限定されていた。
  5日空くと素通りする。ファイルのバイトが違えば `content_hash` の一意制約にも掛からない。
- 対処: `SEMANTIC_DUP_WINDOW_DAYS = 400`。判定条件は従来どおり
  「取引日・金額・通貨・取引先がすべて一致」なので、月額課金（毎月日付が違う）は
  誤検知しない。年次請求の重複も拾えるようになった。
- 台帳画面に「重複チェック」ボタンを追加（`mfScanDuplicates`、**読み取り専用**）。
  全件を取得し、①中身が完全同一(content_hash) ②取引日・金額・通貨・取引先が同一、
...
### 【不具合修正】マッチ待ちの外貨証憑が永久にマッチしなかった
- 症状: 仕訳を登録してから「マッチング実行」を押しても
  「マッチ待ちのまま様子見中: 1件」となり、候補すら出なかった。
- 調査: MFの仕訳を実際に読むと該当仕訳は**存在していた**
  （2026-07-13 / 摘要「VERCEL INC.」/ 通信費3,056+税306 / 未払金3,362）。
- 原因: `processAwaitingMatch` が `findCandidates`（日付+金額の完全一致・円のみ）
  しか使っておらず、`runAutoMatch` にある2段目・3段目の候補提示
  （`findFuzzyCandidates` / `findVendorDateCandidates`）が**丸ごと欠けていた**。
  外貨建ては金額を円と比較できず完全一致が原理的に0件になるため、
  仕訳が実在しても候補が一切表示されないまま待ち続ける状態だった。
  さらに仕訳の取得ウィンドウが `FUZZY_MARGIN_DAYS`(14日) と狭く、
  取引先名＋日付±45日の候補を探しても仕訳が範囲外で取れていなかった。
- 修正:
  - 取得ウィンドウを `MARGIN_DAYS`(=`VENDOR_DATE_MARGIN_DAYS` 45日) に統一
  - 完全一致が決まらなかったとき `findFuzzyCandidates` →
    `findVendorDateCandidates` の順で候補を出し `ambiguous` として返す
  - **自動添付はしない**（金額を検証していないため添付可否は利用者判断）
  - フロントは通常分とマッチ待ち分の候補をまとめて表示
- あわせて候補の説明文が実際の日数と食い違っていたのを修正
  （「±7日」→「±45日」、「±3日」→「±14日」）。定数は45日と14日が正。
- 退行防止として、円建てで日付・金額が完全一致する場合は従来どおり
  自動添付されることもテストで固定した。

### 【不具合修正】取引先名の照合が長い摘要を取りこぼし、別取引を候補に出していた
- 症状: 証憑「Anthropic, PBC / 2026-07-31 / 110 USD」に対し、候補として
  `2026-07-08 1,851円 ANTHROPIC` と `2026-07-09 926円 ANTHROPIC` の**別取引だけ**が出た。
  本命の `2026-07-31 18,644円 ANTHROPIC* CLAUDE SUB` は候補に出なかった（仕訳は実在）。
- 原因: `normalizeText` は空白しか落とさないため、
  `"anthropic,pbc"` と `"anthropic*claudesub"` はどちらも相手を含まず不一致。
  一方、摘要がちょうど `"anthropic"` の別取引は `"anthropic,pbc".includes("anthropic")`
  で一致してしまう。**表記が短い方ほど当たりやすい**という逆転が起きていた。
- 修正（`findVendorDateCandidates` のみ。候補提示専用で自動添付はしない段）:
  - `vendorTokens()` を追加。記号で語に分割し、法人格・汎用語
    （inc/ltd/co/pbc/sub/株式会社 等 = `VENDOR_STOPWORDS`）を落とし、
    英数3文字未満・日本語2文字未満の語も落とす
  - 従来の部分一致に加えて「語がひとつでも共通」でも候補に含める
  - 同じ取引先の取引が複数あるときは**取引日が近い順に並べる**（本命が先頭に来る）
- **`resolveByVendor`（自動添付の判定に使う方）は変更していない**。
  こちらを緩めると誤った自動添付が起きるため、意図的に従来の部分一致のまま。
- 誤検知しないことも固定: 「株式会社」だけの一致、「Inc/Ltd/Co」だけの一致、
  2文字英字だけの一致では候補にしない。

### 月締めが構造上できなくなっていたのを解消（証憑カバー率を参考表示に）
- 症状: 月締めチェックリストで「月締めを完了する」が押せない。
- 原因: `coverageOk`（証憑カバー率100%）が締めの条件に入っていた。
  `computeCoverage` は**その月の全仕訳**を分母にしており、絞り込みが無い。
  実データ(2026-07)は仕訳64件中、証憑ありは4件で6.1%。証憑が無い60件の内訳は
  普通預金/売掛金(回収)12件、支払手数料/普通預金(振込手数料)10件、振替10件、
  普通預金/売上高5件、受取利息・受取配当金4件など、**別途の証憑が存在しない仕訳**。
  つまり100%は構造上あり得ず、月締めが永久にできない状態だった。
- 対処（利用者判断）: カバー率は**参考表示**にして締めをブロックしない。
  締めの可否は `matchingPending === 0 && boxTodo === 0 && shipUnmatch === 0` の
  3項目だけで判定する。どの仕訳に証憑が要るかは会計上の判断のため、
  アプリが一律に強制しない方針。
- `computeCoverage` 自体は変更していない（数字の意味は変えない）。
  将来、分母から銀行取引などを除外する場合は税理士確認が前提。

### そのほか
- `supabase_tax_docs.sql` / `supabase_tax_share.sql` が0バイトに空になっていたのを
  gitから復元（本番適用済みSQLの記録のため中身が必要）。

## 2026-08-01〜02 取込まわりの大規模改修（実運用で見つかった不具合の連続対応）

実際に運用しながら報告→修正を繰り返した長時間セッション。コミット42件。
**追加した機能が既存の弱点を次々に顕在化させた**のが特徴で、以下は
「新機能が引き金となって発覚した既存バグ」を多く含む。

### 追加した機能

- **受信箱（`pages/auto-inbox.js`）**: ダウンロードフォルダ等を一度指定すると、
  取込ページを開くだけで新着CSV・証憑を自動検出して一覧表示する。
  File System Access API（Chrome/Edge のPC版のみ）。ハンドルはIndexedDBに保存。
  取込済み判定はファイル内容のSHA-256で行い、台帳 `ribre_inbox_seen_v1` に記録。
  **取込は必ず利用者のクリック**（自動取込はしない）。
  取込済みファイルは折りたたみで一覧表示し、**ファイル単位で「もう一度取り込む」**
  ことができる（台帳の全消しは配送CSVまで巻き込むため非推奨）。
- **棚番からの出品アカウント自動判定**: 商品タイトル先頭の棚番
  `<配送サイズ><アカウント英字><種別><棚番>` （例 `1HC1`）から
  ヤフオク1〜8を判別する。詳細はCLAUDE.mdの制約参照。
- **取込履歴とチャネル一括訂正（`pages/import-history.js`）**: 取込元を間違えた分を
  CSV単位でまとめてチャネル変更・削除できる。件数上限なし（300件超が本来の用途）。
- **AI質問・修正提案・学習メモ**（`pages/ai-assistant.js` / `ai-write.js` / `ai-memory.js`）:
  分析ページで日本語の質問に答える。修正・削除は「提案→人の承認→自動バックアップ→実行」。
  金額の計算は必ずJS側で行いモデルには暗算させない。モデルは gpt-5.6-luna。
- **配送データのクラウド同期（`pages/shipping-sync.js`）**: 端末をまたいで同じ照合結果にする。
- **MFのMCPサーバー接続**: Claude Codeから会計データを直接照会できる（別途OAuth）。
- 税理士送付ファイルのプレビュー、取引ページのExcelを税理士送付へ直接保存、
  配送照合の「再照合する」（ファイル不要）。

### 直した不具合（実運用で発覚）

1. **取込元の選び忘れで8件が誤登録**: 取込元は前回の選択が残るため、ファイルだけ
   選び替えると気づけなかった。CSVの中身（棚番）とファイル名から推定し、
   選択と食い違えば警告する（`appvGuessImportAccount`）。
2. **保存容量オーバーで取込失敗**: localStorageが5MBに到達。実測は
   配送行1535KB・売上1403KB・yahoo240 1403KB（売上の複製）・照合結果646KB。
   配送行のraw（CSV1行）を照合に使う6列だけ残す圧縮（1225KB→317KB）、
   照合結果から未使用フィールド削除（414KB→172KB）、スナップショットの
   sales/yahooSales重複解消、容量不足時の自動復旧（`ribreFreeStorage`→
   `appvCompactStorage`）を実装。
3. **配送照合でブラウザがOut of Memoryでクラッシュ**: 「売上取込後に自動照合」を
   足したことが引き金。一致行ごとに `appvSyncYahooShip` を呼び、毎回yahoo240
   （約1.4MB）を全読み書きしていた（2861行×2.8MB≒8GB）。一括同期
   `appvSyncYahooShipBatch` に変更し、照合本体も正規化の使い回し＋伝票番号の
   Map索引で最適化（配送3000×売上3500が77ms）。
4. **ヤマトの送料が入らない**: ヤマトは2種のCSVを組み合わせて初めて送料が入る
   （発行済データ=商品ID↔伝票番号／運賃明細=伝票番号↔送料）。処理順が保存順
   依存で、運賃明細が先だと伝票番号がまだ無く突合できなかった。商品IDを持つ行を
   先に処理し、ループ中に付与した伝票番号を索引へ即反映するよう修正。
5. **送料0なのに「配送CSV一致」で埋もれる**: 送り状だけ当たった行は送料0のまま
   このステータスになるが、`appvNeedsShip` が「送料不要」とみなし不一致にも
   数えていなかった。「**送料待ち**」として区別し警告色で不一致に数える。
   これにより「運賃明細の取込漏れ」が可視化されるようになった
   （実際に6/30発送分の運賃明細が未出力だったことが判明した）。
6. **並び順が日付順に崩れる**: 真因は **CSVの行番号がクラウドに保存されていなかった**
   こと。hydrate()がクラウドの内容でlocalStorageを丸ごと置き換えるため order が
   毎回消えていた。`sales.csv_order` 列を追加して同期し、表示時に毎回並べ替える
   方式へ変更（保存側の並びに依存しない）。※原因を3回取り違えた。
7. **審査中の行が取り込まれる**: 受取連絡待ち・キャンセル・現金振り込みは除外して
   いたが「審査中」が漏れていた。
8. **配送CSVの種類を誤判定**: 「発行済データ」をヤマト送り状ではなく佐川と判定して
   いた。中身の列位置による判定の限界のため、ファイル名の手がかりを優先する
   `appvDetectShipTypeByName` を追加。
9. **メルカリShopsをヤフオクと誤認する危険**: メルカリShopsのCSVはファイル名に販路が
   入らず（`202607-202607_report.csv`）、商品名に棚番があるとヤフオクと誤判定
   しうる。ファイル名でヤフオク以外の販路が分かる場合はそれを最優先にした。
10. **取込ページのスクロール地獄**: 配送照合が3000行超を常に全描画していた。
    100件ずつ＋「さらに表示」に変更。取込履歴カードは最下部・折りたたみへ移動。

### 調査して「不具合ではなかった」と確認したもの（記録）

- **受取連絡待ちの除外**は正常動作していた（実データで4件すべて未取込を確認）。
- **6/30発送分の送料が入らない**のは、運賃明細CSVの出力期間に隙間があったため
  （6月分は05/29〜06/29、7月分は07/01〜07/31で6/30が欠落）。利用者が6/30から
  再出力して解消。**システム側の不具合ではない**。
- **3月分が「匿名配送」表示**になるのは、333件が手入力で送料を入れていたため
  （「送料はあるが配送CSV由来ではない」の意味でこのラベルになる）。データは正常。
  過去データのため表示ラベルの修正は見送り（利用者判断）。

### 適用済みSQL（本番実行済み）

- `supabase_sales_csv_order.sql` — sales に csv_order 列（並び順の維持）
- `supabase_shipping_rows.sql` — 配送データのクラウド同期テーブル
